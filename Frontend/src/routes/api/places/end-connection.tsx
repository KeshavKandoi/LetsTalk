...
import { endCurrentConnection } from '@backend/lib/app-state'
import { auth } from '@backend/lib/auth'

export const Route = createFileRoute('/api/places/end-connection')({
  server: {
    handlers: {
      POST: async ({ request }) => {
        try {
          ...
          const result = await endCurrentConnection({ viewerUserId: session.user.id })
