...
import { previewScanJoin } from '@backend/lib/app-state'
import { auth } from '@backend/lib/auth'

export const Route = createFileRoute('/api/places/scan-preview')({
  server: {
    handlers: {
      POST: async ({ request }) => {
        try {
          ...
          const result = await previewScanJoin({ ...body, viewerUserId: session.user.id })
